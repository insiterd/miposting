...
1. **`temporal` con permisos de bind-mount rotos**: `/data/coolify/applications/flofb07yh5gfcgxhw0ko0m1j/dynamicconfig` en el host es propiedad de `coolify` (uid 1001, modo 750/640), pero el proceso dentro del contenedor `temporalio/auto-setup` corre como uid 1000 (`temporal`) — ni dueño ni grupo coinciden, "other" sin permisos → `permission denied` leyendo `development-sql.yaml`. `temporal` quedó en crash-loop, y como `postiz` depende de `temporal` healthy, nunca arrancó. **Fix temporal aplicado en su momento**: `chmod o+rx` en el directorio y `chmod o+r` en el archivo.
   ...
   **Fix definitivo**: se reemplazó el bind-mount (`volumes: - ./dynamicconfig:/etc/temporal/config/dynamicconfig`) por un [Compose `config` de archivo](https://docs.docker.com/reference/compose-file/configs/) con `mode: 0444` explícito en el servicio `temporal` de `docker-compose.yaml`. Docker copia el contenido dentro del contenedor con ese modo al crear el contenedor (leyendo el archivo fuente como root), sin importar los permisos que Coolify le ponga al archivo en el host. Esto hace que el problema no pueda volver a ocurrir sin necesidad de tocar nada en el VPS. Si en el futuro se necesita añadir más archivos de config a otros servicios con el mismo problema, seguir el mismo patrón (`configs:` a nivel de servicio + bloque `configs:` a nivel raíz del compose) en vez de bind-mounts para archivos de solo lectura.
